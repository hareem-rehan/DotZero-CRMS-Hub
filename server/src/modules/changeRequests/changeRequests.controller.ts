...
export const list = async (req: Request, res: Response, next: NextFunction): Promise<void> => {
  try {
    ...
    const result = await crService.listCRs(req.user?.userId as string, req.user?.role as string, {
      ...
      initiatedByDm:
        initiatedByDm === 'true' ? true : initiatedByDm === 'false' ? false : undefined,
    });
...
export const createDMInitiated = async (
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> => {
...
export const dmEditDraft = async (
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> => {
  try {
    const cr = await crService.updateDMDraft(
      req.params['id'] as string,
      req.body,
      req.user!.userId,
    );
...
export const clientConfirm = async (
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> => {
  try {
    const cr = await crService.clientConfirmCR(
      req.params['id'] as string,
      req.user!.userId,
      req.body,
    );
...
export const clientReviewEdit = async (
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> => {
  try {
    const cr = await crService.clientReviewEditCR(
      req.params['id'] as string,
      req.user!.userId,
      req.body,
    );
...
export const clientReject = async (
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> => {
  try {
    const cr = await crService.clientRejectCR(
      req.params['id'] as string,
      req.user!.userId,
      req.body,
    );
...
export const poResubmitWithEdits = async (
  ...
  try {
    const cr = await crService.poResubmitWithEdits(
      req.params['id'] as string,
      req.user!.userId,
      req.body,
    );
...
export const dmReviewResubmit = async (
  ...
  try {
    const cr = await crService.dmReviewResubmit(
      req.params['id'] as string,
      req.user!.userId,
      req.body,
    );
...
export const markInProgress = async (
  ...
  try {
    const cr = await crService.markInProgress(
      req.params['id'] as string,
      req.user!.userId,
      req.user!.role,
    );
...
export const markCompleted = async (
  ...
  try {
    const cr = await crService.markCompleted(
      req.params['id'] as string,
      req.user!.userId,
      req.user!.role,
    );
...
export const recallFromClient = async (
  req: Request,
  res: Response,
  next: NextFunction,
): Promise<void> => {
