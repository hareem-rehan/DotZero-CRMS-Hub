...
export const listFinanceCRs = async (query: {
  ...
  const totals = enriched.reduce(
    ...
    {} as Record<
      string,
      { currency: string; totalCRs: number; totalHours: number; totalCost: number }
    >,
  );
...
export const getFinanceDashboard = async (dateFrom?: string, dateTo?: string) => {
  ...
  const byProject = projectBreakdown.reduce(
    (acc, cr) => {
    ...
    await Promise.all([
      ...
      prisma.changeRequest.findMany({
        where: {
          ...
          status: { in: ['APPROVED', 'IN_PROGRESS', 'COMPLETED'] as CRStatus[] },
        },
        include: {
