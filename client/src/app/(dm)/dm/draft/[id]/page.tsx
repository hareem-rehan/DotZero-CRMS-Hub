  ...
  if (!cr)
    return (
      <PageWrapper title="Not Found">
        <div className="text-sm text-[#5D5B5B]">CR not found.</div>
      </PageWrapper>
    );
  if (!cr.initiatedByDm)
    return (
      <PageWrapper title="Not Found">
        <div className="text-sm text-[#5D5B5B]">Not a DM-initiated CR.</div>
      </PageWrapper>
    );
  ...
  return (
    ...
    >
      ...
      <div className="mb-4 rounded-lg border border-blue-100 bg-blue-50 px-4 py-3 text-sm text-blue-800">
        This CR will be sent to <strong>{cr.submittedBy?.name}</strong> for review before entering
        the approval cycle.
      </div>
          ...
          <div className="rounded-xl border border-[#E5E5E5] bg-white p-6">
            <div className="mb-4 flex items-center justify-between">
              ...
              <div className="flex items-center gap-2">
                ...
                <CRStatusBadge
                  status={cr.status}
                  overrides={{ DRAFT: { label: 'Draft', variant: 'gray' } }}
                />
              </div>
            </div>
          ...
          <div className="rounded-xl border border-[#E5E5E5] bg-white p-5">
            ...
            {editingNotes ? (
              <div className="space-y-2">
                ...
                <div className="flex gap-2">
                  <button
                    ...
                    className="rounded-lg bg-[#EF323F] px-3 py-1.5 text-xs font-semibold text-white"
                  >
                    Save
                  </button>
                  <button
                    onClick={() => setEditingNotes(false)}
                    className="rounded-lg border border-[#D3D3D3] px-3 py-1.5 text-xs text-[#5D5B5B]"
                  >
                    Cancel
                  </button>
                </div>
              ...
              <p className="text-sm text-[#5D5B5B]">
                {cr.dmNotes || <span className="italic text-[#D3D3D3]">No notes</span>}
              </p>
            )}
          </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/40 p-4">
          <div className="w-full max-w-md rounded-xl bg-white p-6 shadow-2xl">
            ...
            <p className="mb-6 text-sm text-[#5D5B5B]">
              This will notify <strong>{cr.submittedBy?.name}</strong> to review the CR. They can
              confirm or reject it back to you.
            </p>
            <div className="flex justify-end gap-3">
              <button
                onClick={() => setConfirmSend(false)}
                className="rounded-lg border border-[#D3D3D3] px-4 py-2 text-sm font-medium text-[#5D5B5B]"
              >
                Cancel
              </button>
              <button
