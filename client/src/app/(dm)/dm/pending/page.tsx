...
function DaysPending({ dateStr }: { dateStr: string | null }) {
  ...
  const cls = days >= 5 ? 'text-red-600 font-semibold' : days >= 2 ? 'text-amber-600 font-medium' : 'text-[#5D5B5B]';
...
export default function DmPendingPage() {
  ...
  const columns: Column<CRSummary>[] = [
    ...
    },
    {
      ...
      header: 'Title',
      render: (row) => (
        <Link href={`/dm/pending/${row.id}`} className="text-sm text-[#2D2D2D] hover:text-[#EF323F] line-clamp-1">
          {row.title}
        </Link>
      ),
    },
