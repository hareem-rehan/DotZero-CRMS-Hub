...
function ResetPasswordForm() {
  ...
  if (!token) {
    return (
      <div className="text-center">
        <p className="text-sm text-[#EF323F]">Invalid or missing reset token. Please request a new reset link.</p>
      </div>
    );
  }
  ...
  return (
    <>
      ...
      <p className="text-sm text-[#5D5B5B] mb-6">Must be at least 8 characters with 1 uppercase and 1 number.</p>
      ...
      <form onSubmit={handleSubmit(onSubmit)} className="flex flex-col gap-4">
        <div className="flex flex-col gap-1">
          <label htmlFor="password" className="text-sm font-medium text-[#2D2D2D]">New password</label>
          <div className="relative">
            ...
            />
            <button type="button" onClick={() => setShowPassword(v => !v)} className="absolute right-3 top-1/2 -translate-y-1/2 text-[#5D5B5B]" tabIndex={-1}>
        ...
        <div className="flex flex-col gap-1">
          <label htmlFor="confirmPassword" className="text-sm font-medium text-[#2D2D2D]">Confirm password</label>
          <input
            id="confirmPassword"
            type="password"
            placeholder="••••••••"
          ...
          />
          {errors.confirmPassword && <p className="text-xs text-[#EF323F]">{errors.confirmPassword.message}</p>}
        </div>
