...
import { useResetPassword, useResetTokenInfo } from '@/hooks/useAuth';
...
function ResetPasswordForm() {
  ...
  const { data: tokenInfo, isLoading: infoLoading } = useResetTokenInfo(token);
  const isSetup = tokenInfo?.purpose === 'setup';
  ...
  if (infoLoading) {
    return <div className="text-sm text-[#5D5B5B] text-center py-4">Loading…</div>;
  }
  ...
  return (
    <>
      ...
      <p className="text-sm text-[#5D5B5B] mb-6">
        {isSetup
          ? 'Set up a strong password to activate your account.'
