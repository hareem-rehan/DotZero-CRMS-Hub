...
function ChangeChip({ value }: { value: number | null }) {
  ...
  return (
    <span className={`ml-2 rounded-full px-2 py-0.5 text-xs font-medium ${positive ? 'bg-green-100 text-green-700' : 'bg-red-100 text-red-700'}`}>
      {positive ? '+' : ''}{value}%
    </span>
  );
}
...
export default function FinanceDashboardPage() {
  ...
  const { data, isLoading } = useDashboard({ dateFrom: dateFrom || undefined, dateTo: dateTo || undefined });
          ...
          {thisPeriod.map((period) => {
            ...
            return (
              <div key={period.currency} className="space-y-4">
                <h2 className="text-sm font-semibold text-[#5D5B5B] uppercase tracking-wide">{period.currency}</h2>
                <div className="grid grid-cols-1 gap-4 sm:grid-cols-3">
                  <div className="rounded-xl border border-[#E5E5E5] bg-white p-5 shadow-sm">
                    <p className="text-xs text-[#5D5B5B]">Approved CRs</p>
                    <p className="mt-1 text-3xl font-bold text-[#2D2D2D]">{period.count}
                  ...
                  <div className="rounded-xl border border-[#E5E5E5] bg-white p-5 shadow-sm">
                    ...
                    <p className="mt-1 text-3xl font-bold text-[#2D2D2D]">{period.hours}h
                  ...
                  <div className="rounded-xl border border-[#E5E5E5] bg-white p-5 shadow-sm">
                    ...
                    <p className="mt-1 text-2xl font-bold text-[#EF323F]">{fmt(period.cost, period.currency)}
                      <ChangeChip value={pct(period.cost, prev?.cost ?? 0)} />
                    </p>
                    <p className="mt-1 text-xs text-[#5D5B5B]">Last month: {fmt(prev?.cost ?? 0, period.currency)}</p>
                  </div>
                </div>
              </div>
            );
                ...
                <thead className="bg-[#F7F7F7]">
                  <tr>
                    {['Project', 'Client', 'CRs', 'Hours', 'Total Cost', 'Currency'].map((h) => (
                      <th key={h} className="px-4 py-3 text-left text-xs font-semibold text-[#5D5B5B] uppercase tracking-wide">{h}</th>
                    ))}
                  </tr>
                </thead>
                <tbody className="divide-y divide-[#F0F0F0]">
                  {breakdown.map((row) => (
                    <tr key={row.projectId} className="hover:bg-[#FAFAFA]">
                      ...
                      <td className="px-4 py-3 font-semibold text-[#2D2D2D]">{fmt(row.cost, row.currency)}</td>
