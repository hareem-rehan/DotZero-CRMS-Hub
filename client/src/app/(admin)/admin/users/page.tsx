...
import { useUsers, useSetUserActive, useResendWelcome, useAdminResetPassword, useDeleteUser, User } from '@/hooks/useUsers';
...
function ActionsDropdown({ row, onToggle, onDelete, onResend, onReset }: {
  row: User;
  onToggle: () => void;
  onDelete: () => void;
  onResend: () => void;
  ...
  return (
    <div className="relative" ref={ref}>
      ...
      >
        Actions
        <svg className="h-3.5 w-3.5 text-[#5D5B5B]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
      ...
      {open && (
        <div className="absolute right-0 z-50 mt-1 w-44 rounded-lg border border-[#E5E5E5] bg-white shadow-lg">
          ...
          >
            <svg className="h-4 w-4 text-[#5D5B5B]" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export default function UsersListPage() {
  ...
  const columns: Column<User>[] = [
    ...
    { key: 'email', header: 'Email', sortable: true, render: (row) => <span className="text-sm text-[#5D5B5B]">{row.email}</span> },
    { key: 'role', header: 'Role', render: (row) => <RoleBadge role={row.role} /> },
    {
      key: 'isActive', header: 'Status',
      render: (row) => <Badge label={row.isActive ? 'Active' : 'Inactive'} variant={row.isActive ? 'green' : 'gray'} />,
    },
    {
      key: 'passwordSetAt', header: 'Password',
      render: (row) => row.passwordSetAt ? <Badge label="Set" variant="green" /> : <Badge label="Pending" variant="yellow" />,
    },
    {
  ...
  return (
    ...
    >
      <div className="mb-4 flex flex-wrap gap-3">
        ...
        />
        <Select value={role} onChange={(e) => setRole(e.target.value)} options={ROLE_FILTER} className="w-44" />
        <Select value={isActive} onChange={(e) => setIsActive(e.target.value)} options={STATUS_FILTER} className="w-36" />
      </div>

      <DataTable columns={columns} data={data?.users ?? []} keyField="id" loading={isLoading} emptyMessage="No team members found." />
