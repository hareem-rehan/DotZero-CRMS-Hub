...
function statusLabel(inv: Invitation) {
  ...
  if (new Date(inv.expiresAt) < new Date()) return { label: 'Expired', className: 'bg-gray-100 text-gray-500' };
...
export default function InvitationsPage() {
  ...
  const sendMutation = useMutation({
    mutationFn: () => apiClient.post('/invitations', { email: sendEmail, projectId: sendProject, role: 'PRODUCT_OWNER' }),
    onSuccess: () => {
      queryClient.invalidateQueries({ queryKey: ['invitations'] });
  ...
  const handleSend = () => {
    if (!sendEmail.trim()) { setSendError('Email is required.'); return; }
    if (!sendProject) { setSendError('Project is required.'); return; }
    setSendError('');
    sendMutation.mutate();
  };
      ...
      <div className="space-y-6">
        ...
        {resendResult && (
          <div className="rounded-xl border border-[#EF323F]/30 bg-[#EF323F]/5 p-4 flex items-start gap-3">
            ...
            <button onClick={() => setResendResult(null)} className="shrink-0 text-[#5D5B5B] hover:text-[#2D2D2D] text-lg leading-none">&times;</button>
          </div>
        )}
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-[#E5E5E5] bg-[#F7F7F7]">
                  <th className="px-4 py-3 text-left text-xs font-semibold text-[#5D5B5B] uppercase tracking-wide">Email</th>
                  ...
                  <th className="px-4 py-3 text-left text-xs font-semibold text-[#5D5B5B] uppercase tracking-wide">Sent By</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-[#5D5B5B] uppercase tracking-wide">Expires</th>
                  <th className="px-4 py-3 text-left text-xs font-semibold text-[#5D5B5B] uppercase tracking-wide">Actions</th>
                </tr>
              </thead>
              <tbody>
                {(data ?? []).length === 0 ? (
                  <tr><td colSpan={6} className="px-4 py-8 text-center text-[#5D5B5B]">No invitations found.</td></tr>
                ) : (data ?? []).map((inv) => {
                  const status = statusLabel(inv);
                  const isPending = !inv.usedAt && new Date(inv.expiresAt) >= new Date();
                  return (
                    <tr key={inv.id} className="border-b border-[#E5E5E5] last:border-0 hover:bg-[#F7F7F7]">
                      <td className="px-4 py-3 font-medium text-[#2D2D2D]">{inv.email}</td>
                      <td className="px-4 py-3 text-[#5D5B5B]">
                        <span className="font-mono text-xs text-[#EF323F]">{inv.project.code}</span>
